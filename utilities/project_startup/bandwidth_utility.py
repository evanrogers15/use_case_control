...
def main(args):
    ...
    for node in filtered_nodes:
        ...
        if args.action == 'add':
            command = f"python3 /etc/frr/bandwidth_adjuster.py {args.action} eth2 50"
            output = run_telnet_command(args.server, console_port, command)
            print(f"Output for {node ['name']}:\n{output}")
            command = f"python3 /etc/frr/bandwidth_adjuster.py {args.action} eth3 50"
            output = run_telnet_command(args.server, console_port, command)
            print(f"Output for {node['name']}:\n{output}")
        elif args.action == 'remove':
            command = f"python3 /etc/frr/bandwidth_adjuster.py {args.action} eth2"
            output = run_telnet_command(args.server, console_port, command)
            print(f"Output for {node ['name']}:\n{output}")
            command = f"python3 /etc/frr/bandwidth_adjuster.py {args.action} eth3"
            output = run_telnet_command(args.server, console_port, command)
            print(f"Output for {node['name']}:\n{output}")
